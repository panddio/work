...
char send_msg(unsigned char msg_buf[], int raw_fd, struct sockaddr_ll sll)
{
	...
	unsigned char src_mac[6] = {0x74,0x27,0xea,0xb3,0x66,0xca}; //柱琦 XP mac
	unsigned char dst_mac[6] = {0xec,0xa8,0x6b,0xac,0xcf,0xf9}; //me XP mac
	...
	ip_hdr->saddr = inet_addr("10.221.2.6");
	...
	*(unsigned short *)(fake+10) = htons(8+msg_len); //UDP的长度	
	...
	udp_hdr->check = htons(checksum((unsigned short *)(fake),(12+8+msg_len)/2));
...
int main(int argc, char *argv[])
{
	...
	while(1)
	{
		unsigned char buf[1024] = "1_lbt6_56#128#7427EAB366CA#0#0#0#4000#9:1441790112:szb05:SZ-B05:32:";
		unsigned char msg_buf[1024] = "";		
		printf("please input the message you want to send:\n");
		fgets(msg_buf, sizeof(msg_buf),stdin);
		
		unsigned char gb[1024] = "";
		utf8_to_gb2312(msg_buf, gb);
		strcat(buf, gb);
		send_msg(buf, raw_fd, sll);		
	}
	return 0;
}
